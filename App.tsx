...
const App: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-all duration-300 ${themeClasses.bg} ${themeClasses.text}`}>
      <header className={`px-4 lg:px-12 py-4 flex items-center justify-between sticky top-0 z-50 ${themeClasses.header}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className={`text-[10px] font-bold uppercase tracking-widest ${themeClasses.textMuted}`}>Terminal Inteligente</p>
          </div>
        </div>

        <nav className={`hidden lg:flex items-center p-1 rounded-2xl ${themeClasses.navBg}`}>
          <TabButton active={activeTab === 'market'} onClick={() => setActiveTab('market')} icon={<FileSpreadsheet size={16}/>} label="Mercado" themeClasses={themeClasses} />
          <TabButton active={activeTab === 'graphs'} onClick={() => setActiveTab('graphs')} icon={<PieChart size={16}/>} label="Gráficos" themeClasses={themeClasses} />
          <TabButton active={activeTab === 'news'} onClick={() => setActiveTab('news')} icon={<Newspaper size={16}/>} label="Notícias" themeClasses={themeClasses} />
          <TabButton active={activeTab === 'transactions'} onClick={() => setActiveTab('transactions')} icon={<X size={16} className="rotate-45" />} label="Lançamentos" themeClasses={themeClasses} />
          <TabButton active={activeTab === 'tax'} onClick={() => setActiveTab('tax')} icon={<FileText size={16}/>} label="Relatório IR" themeClasses={themeClasses} />
        </nav>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
             {/* Corrected: replaced non-existent toggleTheme with setTheme */}
             <button onClick={() => setTheme('light')} className={`p-1.5 rounded-lg ${theme === 'light' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-400'}`}><Sun size={16}/></button>
             <button onClick={() => setTheme('dark')} className={`p-1.5 rounded-lg ${theme === 'dark' ? 'bg-slate-700 text-blue-400' : 'text-slate-400'}`}><Moon size={16}/></button>
             <button onClick={() => setTheme('neon')} className={`p-1.5 rounded-lg ${theme === 'neon' ? 'bg-cyan-500 text-black' : 'text-slate-400'}`}><Zap size={16}/></button>
          </div>
        </div>
      </header>

      <main className={`flex-1 p-4 lg:p-12 max-w-[1600px] mx-auto w-full transition-opacity duration-500 ${isRefreshing ? 'opacity-50' : 'opacity-100'}`}>
